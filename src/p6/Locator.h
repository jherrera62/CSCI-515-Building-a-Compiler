...
class Locator {
  ...
  public:
    ...
    virtual void mutate(const Animation_code*){throw intrinsic_type;}
    virtual void mutate(const Statement*){throw intrinsic_type;}
...
class Animation_block_locator : public Locator {
  public:
    Animation_block_locator(Animation_code* data): Locator(GPL::ANIMATION_CODE), data(data) {}
    virtual void mutate(const std::string& val);
