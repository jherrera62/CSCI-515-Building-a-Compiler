...
program:
    declaration_list block_list
    {
        ...
        std::set_difference(Animation_code::used_blocklist.begin(), Animation_code::used_blocklist.end(), Animation_code::defined_blocklist.begin(), Animation_code::defined_blocklist.end(), std::inserter(used, used.begin()));
        for(auto i : used)
        {
        ...
        std::set_difference(Animation_code::declared_blocklist.begin(), Animation_code::declared_blocklist.end(), Animation_code::defined_blocklist.begin(), Animation_code::defined_blocklist.end(), std::inserter(set, set.begin()));
        for(auto i:set)
        {
            sm.erase(i);
        }
        ...
        while(par)
        {
            try
            {
                switch(gop->attribute_type(par->name))
                {
                    ...
                    {
                        if(par->value->evaluate()->as_animation_block()->get_parameter_type()!=$1)
                        {
                            Error::error(Error::GAME_OBJECT_ANIMATION_BLOCK_PARAMETER_TYPE_ERROR,GPL::to_string($1),GPL::to_string($3->value->evaluate()->as_animation_block()->get_parameter_type()));
                        }


                        gop->write_attribute(par->name, par->value->evaluate()->as_animation_block());
                        Animation_code::used_blocklist.insert(par->value->evaluate()->as_animation_block()->get_block_name());
                        break;
                    }
                    default:
                        assert(false);
...
initialization_block:
    T_INITIALIZATION statement_block
    {
        ...
        keyvent.add_handler(Window::INITIALIZE,$2);
    }
...
termination_block:
    ...
    {
        Event_manager& keyvent = Event_manager::instance();
        keyvent.add_handler(Window::TERMINATE,$2);
    }
...
forward_declaration:
    ...
    {
        Scope_manager& sm=Scope_manager::instance();
        Animation_code* ptr = new Animation_code(*$3,$5);
        if(sm.defined_in_current_scope(*$3))
        {
          Error::error(Error::PREVIOUSLY_DECLARED_VARIABLE, *$3);
        }
        sm.add_to_current_scope(std::make_shared<Symbol>(*$3, ptr));
...
animation_block:
    animation_declaration statement_block
    {
        ...
        if($1!=nullptr)
        {
            sm.lookup(*$1)->as_lvalue()->mutate($2);
...
animation_declaration:
    ...
    {
        Scope_manager& sm=Scope_manager::instance();
        if(sm.lookup(*$2)==nullptr)
        {
            sm.add_to_current_scope(std::make_shared<Symbol>(*$2, new Animation_code(*$2, $4)));
        }
        if(sm.lookup(*$2)->get_type()!=GPL::ANIMATION_CODE)
        {
        ...
        {
            sm.lookup(*$2)->as_lvalue()->mutate(*$5);
            if($4==sm.lookup(*$2)->as_constant()->as_animation_block()->get_parameter_type())
            {
                $$=$2;
            }
            else
            {
                Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, GPL::to_string($4), GPL::to_string(sm.lookup(*$2)->as_constant()->as_animation_block()->get_parameter_type()));
            }
        }
        Game_object* gop;
        sm.push_table();
        if($4==GPL::RECTANGLE)
        {
            Rectangle* ptr = new Rectangle;
            gop = ptr;
            sm.add_to_current_scope(make_shared<Symbol>(*$5,ptr));
        }
...
assign_statement:
    ...
    {
        if(!($1->type() & (GPL::INT| GPL::DOUBLE | GPL::STRING | GPL::ANIMATION_BLOCK)))
        {
        ...
        {
            GPL::Type type1 = $1->evaluate()->as_animation_block()->get_parameter_type();
            GPL::Type type3 = $3->evaluate()->as_animation_block()->get_parameter_type();
            if(($3->type() & (ANIMATION_CODE | ANIMATION_BLOCK)))
            {
                if(type1 != type3)
                {
                    Error::error(Error::ANIMATION_BLOCK_ASSIGNMENT_PARAMETER_TYPE_ERROR, GPL::to_string(type1), GPL::to_string(type3));
                }
                else if($3->type() == GPL::ANIMATION_CODE )
                {
            ...
            {
                Error::error(Error::ANIMATION_BLOCK_ASSIGNMENT_PARAMETER_TYPE_ERROR, GPL::to_string($1->type()), GPL::to_string($3->type()));
                $$=new NullStatement(); 
            }
            else if(type1!=type3)
            {
...
expression:
    ...
    | expression T_NEAR expression           {$$=Bin_op_check<Near, NEAR>($1,$3, GPL::GAME_OBJECT);}
    | expression T_TOUCHES expression        {$$=Bin_op_check<Touches, TOUCHES>($1,$3, GPL::GAME_OBJECT);}
