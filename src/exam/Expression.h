...
class Ternary_operator : public Expression {
  public:
    Ternary_operator(const Expression* lhs, const Expression* mhs, const Expression* rhs) :lhs(lhs),mhs(mhs), rhs(rhs) {}
    virtual const Constant* evaluate() const=0;
    virtual ~Ternary_operator() { delete lhs;delete mhs; delete rhs; }
  protected:
    const Expression* lhs;
    const Expression* mhs;
...
class Ternary : public Ternary_operator {
  public:
    Ternary(const Expression* lhs, const Expression* mhs, const Expression* rhs) : Ternary_operator(lhs,mhs, rhs){};
    virtual const Constant* evaluate() const;
    virtual GPL::Type type() const;
};
#endif
