...
%{
	...
	const Expression* Tern_op_check(const Expression* one, const Expression* three,const Expression* five, unsigned int valid_types)
	{ 
	bool lhs_valid=one->type() & valid_types;
	bool mhs_valid=three->type() & valid_types;
	bool rhs_valid=five->type() & valid_types;
	if(lhs_valid && rhs_valid && mhs_valid)
		return new OP(one, three, five); 
	if(!lhs_valid) { Error::error(Error::INVALID_LEFT_OPERAND_TYPE,  to_string(one->type())); }
	if(!rhs_valid) { Error::error(Error::INVALID_RIGHT_OPERAND_TYPE, to_string(optype)); }
	// if(!mhs_valid) { Error::error(Error::INVALID_RIGHT_OPERAND_TYPE, to_string(optype)); }
	delete one;
	delete three;
    delete five;
...
//---------------------------------------------------------------------
assign_statement:
    variable T_ASSIGN expression T_TERNARY expression T_COLON expression
    {
        if($3)
        {
            $$=new Assign($1,$5);
...
expression:
    ...
    | expression T_TERNARY expression T_COLON expression {$$=Tern_op_check<Ternary, TERNARY>($1,$3, $5, GPL::INT|GPL::DOUBLE|GPL::STRING);}
