...
1.  Copy the files from ``p4`` into ``p5``:
...
4.  Integrate the ``Print`` class into the grammar actions.  A ``Print`` object is created in the action of the ``print_statement:`` production:
    ...
    Ensure the project compiles.  As with previous projects, "type clash" warnings can be silenced with ``{ $$=nullptr; /*CHANGE*/ }``.
...
10. The ``Symbol`` class has four ``as_constant`` member functions that generate and return ``Constant`` objects. Write four member functions for the ``Symbol`` class that generate and return ``Locator`` objects::
...
11. The ``Variable`` and ``Member_variable`` classes each have an ``evaluate()`` member function that creates and returns a ``Constant`` object for use in expression evaluation. Write the complementary function, ``modify()`` for both ``Variable`` and ``Member_variable`` that returns a shared pointer to a ``Locator`` object. The body of ``modify()`` is almost identical to the body of ``evaluate()``. The ``modify()`` function should be virtual, just as the ``evaluate()`` function is.

|

12. The ``Assign`` class handles simple assignment. GPL Examples::
...
13. In the grammar, create an ``Assign`` object as the action in the first rule of the ``assign_statement:`` production. Review the grammar and ensure the pointer reaches the ``statement_list:`` production.

|

14. The project should now pass tests ``t007.gpl``, ``t008.gpl``, & ``t009.gpl``.

|

15. Implement the four remaining assignment statements: ``+=``, ``-=``, ``++``, and ``--``.

|

16. Ensure the project passes tests:
...
17. Implement the ``if/else``, ``exit``, and ``for-loop`` statements. Consult the GPL manual for usage details. They have the following private data members:
...
18. Initial statement-specific tests:
...
* Converted Statement header to use default argument for ``next``.
* Minor elimination of verbiage.
* Moved tail end of "Locator Hierarchy" box into its own item after box.
* lower/upper case typo.
* make as_lvalue functions ``const``
...
* Changed font on directory names.
